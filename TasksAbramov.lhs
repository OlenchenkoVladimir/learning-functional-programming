...
\begin{flushleft}
\textbf{Студент:} Оленченко Владимир Викторович

\textbf{Группа:} СНТ Эергетик, ст-ца Романовская, Ростовская обл.

\textbf{Вуз:} Университет города Переславля имени~А.~К.~Айламазяна.

\textbf{Дата начала решения задач:} 20.09.2022.
\end{flushleft}
...
\textbf{1-2} Написать функцию \prg{decmls :: Integer {\AR} Integer}, которая вычисляет, сколько существует неотрицательных целых десятичных чисел с \prg{n} цифрами.
...
Написать функцию \prg{balance :: Float {\AR} Float {\AR} Float {\AR} Integer {\AR} Float}, которая выдает задолженность Василия перед банком по истечению $m$ месяцев.
